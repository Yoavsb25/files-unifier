...
from pdf_merger.ui.handlers import _STATE_RUNNING, FileSelectionHandler, MergeHandler
...
class TestFileSelectionHandler:
    ...
    def test_handler_initialization_with_callbacks(self):
        ...
        on_file = MagicMock()
        on_error = MagicMock()
        handler = FileSelectionHandler(on_file_selected=on_file, on_error=on_error)
    ...
    def test_select_directory_success(self, mock_validate, mock_dialog):
        ...
        result = handler.select_directory(
            title="Select Directory", validate=True, folder_type="Source"
        )
    ...
    def test_select_directory_no_validate(self, mock_dialog, tmp_path):
        ...
        handler = FileSelectionHandler(on_file_selected=on_file)
        result = handler.select_directory(title="Select Output", validate=False)
...
class TestMergeHandler:
    ...
    def test_run_merge_missing_paths(self):
        ...
        handler.run_merge(input_file=None, pdf_dir=Path("/pdfs"), output_dir=Path("/output"))
    ...
    def test_run_merge_already_processing(self):
        ...
        handler.run_merge(
            input_file=Path("/test.csv"), pdf_dir=Path("/pdfs"), output_dir=Path("/output")
        )
    ...
    def test_run_merge_success(self, mock_run_merge_job, tmp_path):
        ...
        handler = MergeHandler(on_start=on_start, on_complete=on_complete)
    ...
    def test_run_merge_error(self, mock_run_merge_job, tmp_path):
        ...
        handler = MergeHandler(on_start=on_start, on_error=on_error)
