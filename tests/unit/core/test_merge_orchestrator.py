...
class TestRunMergeJob:
    ...
    def test_run_merge_job_success(self, mock_logger, mock_read_data, mock_process_job, tmp_path):
        ...
        mock_read_data.return_value = iter([{"serial_numbers": "GRNW_123"}])

        # Mock process_job to return result
        expected_result = MergeResult(total_rows=1, successful_merges=1, job_id="test-job")
    ...
    @patch("pdf_merger.core.job_loader.read_data_file")
    @patch("pdf_merger.core.merge_orchestrator.logger")
    def test_run_merge_job_with_fail_on_ambiguous(
        self, mock_logger, mock_read_data, mock_process_job, tmp_path
    ):
    ...
    @patch("pdf_merger.core.job_loader.read_data_file")
    @patch("pdf_merger.core.merge_orchestrator.logger")
    def test_run_merge_job_with_custom_column(
        self, mock_logger, mock_read_data, mock_process_job, tmp_path
    ):
        ...
        mock_read_data.return_value = iter([{"custom_col": "GRNW_123"}])
    ...
    @patch("pdf_merger.core.job_loader.read_data_file")
    @patch("pdf_merger.core.merge_orchestrator.logger")
    def test_run_merge_job_empty_file(
        self, mock_logger, mock_read_data, mock_process_job, tmp_path
    ):
