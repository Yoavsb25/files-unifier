...
from pdf_merger.observability.metrics import Metric, MetricsCollector, get_metrics_collector
...
class TestMetric:
    ...
    def test_metric_creation(self):
        ...
        metric = Metric(
            name="test_metric", value=42.5, timestamp=1234567890.0, tags={"tag1": "value1"}
        )
    ...
    def test_metric_default_tags(self):
        ...
        metric = Metric(name="test_metric", value=10.0, timestamp=1234567890.0)
...
class TestMetricsCollector:
    ...
    def test_export_to_file_writes_valid_json(self, tmp_path):
        """export_to_file writes JSON with timestamp, counters, timers; returns True."""
        collector = MetricsCollector(enabled=True)
        collector.record_counter("jobs_completed", value=1)
        path = tmp_path / "metrics.json"
        result = collector.export_to_file(path)
        assert result is True
        assert path.exists()
        data = json.loads(path.read_text(encoding="utf-8"))
        assert "timestamp" in data
        assert "counters" in data
        assert data["counters"].get("jobs_completed") == 1
        assert "timers" in data

    def test_export_to_file_invalid_path_returns_false_does_not_raise(self, tmp_path):
        """export_to_file with path that cannot be written returns False and does not raise."""
        collector = MetricsCollector(enabled=True)
        # Passing a directory as file path causes open() to fail when writing
        result = collector.export_to_file(tmp_path)
