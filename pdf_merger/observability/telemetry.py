...
class TelemetryService:
    ...
    def __init__(self, enabled: bool = False, endpoint: Optional[str] = None):
        ...
        Args:
            ...
            endpoint: Reserved for future backend; not yet used. Events are stored
                in memory only. When a backend is implemented, record_event may POST
                to this URL when enabled.
        """
    ...
    def flush(self) -> None:
        ...
        if self.endpoint:
            ...
            logger.debug(
                f"Telemetry collected: {len(self.events)} events (not sent, endpoint not configured)"
            )
