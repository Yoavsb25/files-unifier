...
def resolve_required_column(ui_value: Optional[str], config_value: Optional[str]) -> str:
    ...
    return (
        ui_value or config_value or DEFAULT_SERIAL_NUMBERS_COLUMN
    ).strip() or DEFAULT_SERIAL_NUMBERS_COLUMN
...
class AppConfig:
    ...
    def merge(self, other: "AppConfig") -> "AppConfig":
        ...
        crash_reporting_enabled = getattr(
            other, "crash_reporting_enabled", self.crash_reporting_enabled
        )
        fail_on_ambiguous_matches = getattr(
            other, "fail_on_ambiguous_matches", self.fail_on_ambiguous_matches
        )
        return AppConfig(
...
def load_config(start_path: Optional[Path] = None) -> AppConfig:
    """
