...
class Row:
    ...
    @classmethod
    def from_raw_data(cls, row_index: int, raw_data: dict, required_column: str) -> "Row":
        """
        ...
        for serial_number in all_serial_numbers:
            if validate_serial_number(serial_number):
                ...
                logger.warning(
                    f"Row {row_index + 1}: Invalid serial number format: {serial_number}"
                )

        # Deduplicate and normalize
        unique_serial_numbers = deduplicate_serial_numbers(
            valid_serial_numbers, preserve_order=True
        )
        normalized_serial_numbers = [
            normalize_serial_number(s, to_uppercase=True) for s in unique_serial_numbers
        ]
