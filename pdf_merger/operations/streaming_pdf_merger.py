...
def merge_pdfs_streaming(
    pdf_paths: List[Path], output_path: Path, chunk_size: int = STREAMING_CHUNK_SIZE
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Merging {len(pdf_paths)} PDF(s) with {total_pages} total pages using streaming mode"
        )
            ...
            try:
                with suppress_stderr():
                    ...
                    for i in range(0, len(reader.pages), chunk_size):
                        ...
                        if (
                            total_pages > STREAMING_PROGRESS_INTERVAL * 2
                            and processed_pages % STREAMING_PROGRESS_INTERVAL == 0
                        ):
...
def should_use_streaming(
    pdf_paths: List[Path], threshold_mb: float = Constants.STREAMING_THRESHOLD_MB
) -> bool:
    """
