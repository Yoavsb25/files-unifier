...
def build_application(platform: str) -> bool:
    ...
    if platform == "macos":
        ...
        # Run build script
        result = subprocess.run(["bash", str(build_script)], cwd=project_root, capture_output=False)
        ...
        result = subprocess.run(
            [str(build_script)], cwd=project_root, shell=True, capture_output=False
        )
        if result.returncode != 0:
            print_error("Build failed")
            return False
...
def get_customer_info():
    ...
    expires = prompt_for_input(
        "License expiration date (YYYY-MM-DD)", default=default_expires, validator=validate_date
    )

    # Number of machines
    machines_str = prompt_for_input(
        "Number of allowed machines", default="1", validator=validate_machines
    )
    ...
    platform_input = prompt_for_input(
        "Platform (1/2 or macos/windows)", validator=validate_platform
    )
    ...
    skip_build_input = prompt_for_input(
        "Skip building? Use existing build if available (y/n)", default="n"
    )
...
def main():
    ...
    parser.add_argument("--expires", help="License expiration date (YYYY-MM-DD)")
    parser.add_argument("--machines", type=int, help="Number of allowed machines (default: 1)")
    parser.add_argument(
        "--platform", choices=["macos", "windows"], help="Target platform (macos or windows)"
    )
    parser.add_argument(
        "--private-key",
        type=Path,
