...
from pdf_merger.utils.logging_utils import get_logger, setup_logger
...
def generate_keys(output_dir: Path):
    ...
    if save_private_key(private_key, private_key_path):
        ...
        print("  WARNING: Keep this key secure and never share it!")
    else:
    ...
    if save_public_key(public_key, public_key_path):
        ...
        print("  This key should be embedded in the application.")
    else:
...
def generate_license(
    ...
    # Create license
    license = License(
        company=company, expires=expires, allowed_machines=allowed_machines, version=version
    )

    # Sign license
    print("Signing license...")
    if not sign_license(license, private_key):
        print("✗ Error: Failed to sign license")
        return False
    ...
    if license.save_to_file(output_path):
        ...
        print("✗ Error: Failed to save license")
        return False
...
def main():
    ...
    license_parser.add_argument("--company", required=True, help="Company name")
    license_parser.add_argument("--expires", required=True, help="Expiration date (YYYY-MM-DD)")
    license_parser.add_argument(
        "--machines", type=int, default=1, help="Number of allowed machines (default: 1)"
    )
    license_parser.add_argument(
        "--version", default=APP_VERSION, help=f"License version (default: {APP_VERSION})"
    )
    license_parser.add_argument(
        "--private-key",
        type=Path,
